...
- Dockerfile Node 24/bookworm-slim à la racine, aucun changement du serveur ou du moteur. Railway détecte le Dockerfile ; réglages par interface plutôt que deux configurations concurrentes. Aucun railway.json/nixpacks.toml nécessaire. La documentation Railway consultée annonce config-as-code déprécié : https://docs.railway.com/config-as-code/reference.
- Dépendances de développement conservées dans l'image : pg est actuellement déclaré dans devDependencies et les commandes sync existantes relancent tsc. Ne pas npm prune --omit=dev sans changer ces contrats. NODE_ENV=production après le build ; .dockerignore exclut secrets locaux, sorties et logs.
- PostgreSQL/PostGIS séparé via l'image reconnue 17-3.5 déjà utilisée par CI, volume PGDATA dédié, accès privé et mots de passe uniquement dans Railway. Pas de db:reset ni de changement de migrations.
- Précommande du site limitée à migrations + db:check ; synchronisations dans un service de tâche distinct utilisant staging:sync. Les commandes sont séquentielles, échec non masqué, marqueur de réussite seulement à la fin. Les imports déjà terminés ne sont pas annulés si une source ultérieure échoue.
- Correction du blocage d'une base neuve : sync:lyon:boundary ajoute le seul contour officiel Lyon (69123) dans cities, avec validation d'identité/type et contraintes PostGIS existantes. Source API Découpage administratif, URL et instant de récupération dans les logs. Aucun emplacement, autorisation ou nouveau moteur. Le fournisseur est interrogé seulement pendant l'initialisation/synchronisation administrative.
- Absence d'emplacements vérifiés : conserver UNKNOWN ; ne pas désactiver les garde-fous du service pour rendre le site artificiellement démonstratif. Temps réel non activé sans mapping et accès validés. La revue réglementaire a une échéance existante qui n'est pas prolongée ici.
- Trois tests locaux de validation de contour et une intégration PostGIS ajoutés. CI historique conservée, enrichie d'un build Docker et d'un smoke HTTP/readiness de l'image. Docker/PostGIS ne sont ni installés ni exécutés dans Work.
