...
test('Application composes geocoding coverage decision and pricing without persisting address',async()=>{const r=await new CheckParkingService(dependencies()).check('fixture',period,now);assert.equal(r.decision.status,'ALLOWED');assert.equal(r.pricing.status,'PAID');assert.equal(r.confidence,'DOCUMENTED');});
